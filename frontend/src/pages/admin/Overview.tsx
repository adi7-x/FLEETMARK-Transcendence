...
import { getTrips, getRoutes, getStoredUser } from '../../services/api';
...
const Overview = () => {
  ...
  useEffect(() => {
    Promise.all([getTrips(), getRoutes()])
      .then(([t, r]) => { setTrips(t); setRoutes(r); })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14 }}>
        {[
          { emoji: '🎓', label: t('dashboard.nav.students', 'Students'), value: '400+', sub: t('dashboard.admin.registeredStudents', 'Registered'), bg: V.blueBg },
          ...
          { emoji: '🌙', label: t('dashboard.admin.operatingHours', 'Hours'), value: '10PM–6AM', sub: t('dashboard.admin.operatingWindow', 'Operating window'), bg: V.amberBg },
        ].map((s) => (
